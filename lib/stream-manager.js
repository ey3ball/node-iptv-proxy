...
module.exports = {
        ...
        createChan: function (id) {
                var chanEntry = this.findChan(id);
                if (chanEntry) {
                        return chanEntry.idx;
                }
        ...
        addChan: function (id, readableStream, head, stop) {
                var chanEntry = this.findChan(id);
                if (chanEntry) {
                        chanEntry.obj.inputStream = readableStream;
                        chanEntry.obj.headers = head;
                        chanEntry.obj.stop = stop;
                        chanEntry.obj.state = "streaming";

                        console.log("pipe it");

                        if (!chanEntry.obj.clients.length && chanEntry.obj.state == "clientwait") {
                                this.killChan(id);
                                return undefined;
                        }

                        chanEntry.obj.clients.map(function(el) {
                                chanEntry.obj.inputStream
                                        .pipe(new Leaky()).pipe(el);
                        });
                } else {
                        console.log("WARNING: adding unknown chan");
        ...
        addClient: function(chan_id, client_handle, type) {
                ...
                if (chan.state == "streaming")
                        chan.inputStream.pipe(new Leaky()).pipe(client_handle);
                else if (chan.state == "reset") {
                        chan.state = "clientwait";
                }
        ...
        killClient: function(chan_id, handle) {
                ...
                if (!chan.clients.length && chan.state != "reset" && chan.state != "clientwait")
